...
function updateSkills() {
    ...
    skillCounter.innerText = "Characters Remaining: "  + (skillCharLimit - skillInput.value.length)
}
...
function updateSkills() {
    ...
    // check to see if it matches the correct skill format
    const validate = skillInput.value.match(/^[a-zA-Z-_0-9]+$/)
    if (!validate || !validate.length == 1) {
        document.getElementById("skill_error").style = "color:red;";
    } else {
